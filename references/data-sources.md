...
3. MCP (Model Context Protocol) servers: the native way to hook an agent to live data. Two hosted ones matter today: DefiLlama (`claude mcp add --transport http defillama https://mcp.defillama.com/mcp`; free for public data, a Pro API key unlocks premium endpoints through the same connection) and Morpho (docs.morpho.org, developers/agents section: vaults, markets, positions as tools). More protocols are shipping these; check each docs site's "agents" section and the llms.txt index. In Claude apps, add the server URL under connectors/settings. When an MCP is connected, prefer it over scraping: typed tools beat parsed HTML. The MCP connection lives on the USER's machine, not in this skill: with it, use the typed tools; without it, the keyless recipes below cover the same data. Disambiguation: DefiLlama's "LlamaAI" is their subscriber chat product, a UI for humans with no agent API; do not hunt for it, the data lives in the APIs and the MCP.
